...
class urma:
    def template(self):
        ...
        if self.product != "pcp":
            ...
            PATH_OLD = f"urma2p5.{self.date:%Y%m%d}/urma2p5.t{self.date:%H}z.2dvar{self.product}_ndfd.grb2_ext" #older URMA data is in this file format!
            self.SOURCES = {
                "aws": f"https://noaa-urma-pds.s3.amazonaws.com/{PATH}",
                "aws_old" : f"https://noaa-urma-pds.s3.amazonaws.com/{PATH_OLD}",
